...
def load_data(messages_filepath, categories_filepath):
    """
    load datasets messages and categories from the specified filepaths
    Inputs:
...
def clean_data(df):
    """
    clean merged dataset from messages and categories
    Input:
...
def save_data(df, database_filename):
    """
    save cleaned df into an sqlite database
    Inputs:
        df:  cleaned df
        database_filename: name of the database file
    Output:
        db with name database_filename
    """
    engine = create_engine('sqlite:///' + database_filename)
    df.to_sql('CleanData', engine, if_exists='replace', index=False)
